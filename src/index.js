...
const Swipeout = createReactClass({
  ...
  _forceOpen: function() {
    const { sectionID, rowID, onOpen, buttonWidth, left, right } = this.props;
    const btnWidth = buttonWidth || 50;
    const btnsRightWidth = right ? btnWidth * right.length : 0;
    const btnsLeftWidth = left ? btnWidth * left.length : 0;
    const contentPos = left ? btnsLeftWidth : -btnsRightWidth;
